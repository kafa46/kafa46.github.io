...
- `failuers`: 적 격추에 실패하여 적이 화면 하달에 도달한 횟수
    - 10개를 놓치면 실패하도록 설정
- `collisions`: 적과 플레이어가 충돌한 횟수
    - 적과 3번 충돌하면 실패하도록 설정
    ```python
    class ShootingGame:
        def __init__(self):
...
- 게임에서 졌을 경우 메시지를 띄우기 위해 messagebox를 임포트 합니다.
    ```python
    from tkinter import messagebox
    ```

- 게임 종료 상황을 처리하는 메서드 (`check_game_over`)
    
    - `failures`가 10에 도달하면 게임이 종료
        - "Game Over! Your score is {score}. You failed to block 10 enemies." 메시지가 표시

    - `collisions`가 3에 도달하면 게임이 종료
        - "Game Over! Your score is {score}. You collided with enemies 3 times." 메시지가 표시
